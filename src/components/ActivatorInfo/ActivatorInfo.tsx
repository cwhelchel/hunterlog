...
export const ActivatorInfo = (props: IActivatorInfoProps) => {
    ...
    React.useEffect(() => {
        ...
        // this was cleared in some way. entering call manually maybe.
        if (contextData?.spotId == 0) {
            setActComments([]);
            setCwSpeed(0);
        }

        const pSpot = window.pywebview.api.get_spot(contextData?.spotId);

        pSpot.then((r: string) => {
            const spot = JSON.parse(r) as SpotRow;
            if (spot) {
                // pipe delimited
                if (spot.act_cmts) {
    ...
    function copyMarkDown(event: React.MouseEvent<SVGSVGElement>) {
        ...
        const qso = contextData.qso;

        const text = `**${qso.call}** -- ${qso.mode} (_${qso.freq}_) @ park ${qso.sig_info}`;
    ...
    function copyText() {
        ...
        const text = `${qso.call} -- ${qso.mode} (${qso.freq}) @ park ${qso.sig_info}`;
        navigator.clipboard.writeText(text);
    }
    ...
    function openQrzLink() {
        ...
        const url = `http://www.qrz.com/db/${qso.call}`;
        window.open(url);
    }
    ...
    function openRbnLink() {
        ...
        const pskRptUrl = `https://pskreporter.info/pskmap.html?callsign=${qso.call}&mode=${qso.mode}`
        const url = `https://www.reversebeacon.net/main.php?spotted_call=${qso.call}&rows=30&timerange=900`;
